...
// emitted signals
const omnetpp::simsignal_t StaticNodeManager::addRoadSideUnitSignal = omnetpp::cComponent::registerSignal("addRoadSideUnit");
...
void StaticNodeManager::loadRoadSideUnits()
{
    ...
    for (omnetpp::cXMLElement* rsu : config->getChildrenByTagName("rsu")) {
        ...
        if (!id) {
            EV_WARN << "rsu has no id specified, skip" << std::endl;
        } else if (mRsuMap.find(id) != mRsuMap.end()) {
            EV_WARN << "rsu with id " << id << " already exists, skip" << std::endl;
        }
